...
Файл test_auth_flow.py (3 теста)
Этот файл отвечает за безопасность входа в систему:

Успешная регистрация: система позволяет создать нового пользователя с валидными данными.
Дубликаты при регистрации: если попытаться зарегистрировать пользователя с уже занятым email или username, сервер возвращает ошибку 400 Bad Request, а не падает с ошибкой базы данных.
Аутентификация (Login): пользователь может войти в систему по своим учетным данным и получить токен доступа, который затем используется для последующих запросов.

Файл test_cart.py (5 тестов)
Это ядро бизнес-логики. Тесты гарантируют, что корзина ведет себя предсказуемо:

Создание пустой корзины:

Проверяет, что новому авторизованному пользователю сразу создается объект заказа со статусом NEW.
Гарантирует, что сумма равна строке '0.00', а список товаров пуст.

Добавление товара и расчет суммы:
Проверяет создание позиции в корзине.
Валидирует математику: цена умножается на количество (500×3=1500).
Проверка БД: убеждается, что остаток на складе действительно уменьшился (было 10, стало 7).

Защита склада:
Пытается обмануть систему, добавив больше штук, чем есть физически.
Проверяет, что сервер блокирует операцию кодом 400 и пишет понятное сообщение об ошибке «Недостаточно товара на складе».

Удаление позиций:
Проверяет удаление конкретного товара из списка через endpoint удаления.
Контролирует пересчет общей суммы после удаления (была смешанная корзина, осталась одна позиция на 200).

Оформление заказа:
Имитирует нажатие кнопки «Подтвердить заказ».
Проверяет смену статуса с NEW на AWAITING_PAYMENT.
Убеждается, что к заказу успешно привязался контакт клиента (проверка ID контакта).
