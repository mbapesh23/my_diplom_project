...
class StoreViewSet(viewsets.ModelViewSet):
    ...
    # GET запросы разрешены всем залогиненным,
    # а мутации (POST/PUT/DELETE) — только Поставщикам
    permission_classes = [permissions.IsAuthenticated]

    def get_permissions(self):
        # Динамически меняем права: для записи требуем роль поставщика
    ...
    def perform_create(self, serializer):
        # Принудительно привязываем магазин к текущему пользователю
        # Даже если клиент попытается подменить user_id в теле запроса, он проигнорируется
        serializer.save(user=self.request.user)

    def create(self, request, *args, **kwargs):
        # Дополнительная страховка на уровне метода
        if not request.user.groups.filter(name='Supplier').exists():
            return Response(
                {"detail": "Доступ запрещен. Требуется роль Поставщика."}, 
